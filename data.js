const cursos = [
    {
        id: 1,
        titulo: "Programação Web Completo",
        descricao: "Aprenda HTML, CSS, JavaScript, React e Node.js do zero ao avançado. Crie sites profissionais.",
        preco: 100,
        categoria: "Programação",
        duracao: "40 horas",
        nivel: "Iniciante",
        emoji: "💻",
        pixKey: "lucasklein2411@gmail.com",
        codigoAcesso: "PROG2024",
        alunos: 2,
        avaliacao: 10,
        conteudoTexto: `CURSO: PROGRAMAÇÃO WEB COMPLETO
================================

AULA 1: HTML5 E CSS3 FUNDAMENTOS
---------------------------------
Introdução ao HTML5
- Estrutura básica de uma página HTML
- Tags semânticas e sua importância
- Hierarquia de elementos

CSS3 Fundamentos
- Seletores e propriedades básicas
- Box model e layout
- Cores, fontes e espaçamento
- Flexbox e Grid layout

Exercícios práticos:
- Criar sua primeira página HTML
- Estilizar com CSS3
- Layout responsivo básico

AULA 2: JAVASCRIPT ES6+
-----------------------
Introdução ao JavaScript
- Variáveis (let, const, var)
- Tipos de dados
- Operadores e expressões

Funções e Escopo
- Declaração de funções
- Arrow functions
- Escopo de variáveis
- Closures

ES6+ Features
- Template literals
- Destructuring
- Spread e Rest operators
- Promises e async/await

DOM Manipulation
- Seleção de elementos
- Manipulação de conteúdo
- Eventos e listeners

AULA 3: REACT.JS DO ZERO
-------------------------
Introdução ao React
- O que é React e por que usar
- Configuração do ambiente
- Create React App

Componentes
- Componentes funcionais
- Props e State
- Ciclo de vida dos componentes

Hooks
- useState
- useEffect
- useContext
- Custom hooks

React Router
- Navegação entre páginas
- Rotas dinâmicas
- Parâmetros de rota

AULA 4: NODE.JS E EXPRESS
--------------------------
Introdução ao Node.js
- O que é Node.js
- Instalação e configuração
- NPM e gerenciamento de pacotes

Módulos do Node
- fs (file system)
- http
- path
- modules

Express.js
- Configuração do Express
- Rotas e middlewares
- Req e Res
- Template engines

REST API
- Métodos HTTP
- Estrutura de API REST
- JSON e validação

AULA 5: BANCO DE DADOS
-----------------------
Introdução a Bancos de Dados
- SQL vs NoSQL
- Tipos de bancos de dados
- Escolhendo o banco certo

MySQL/MariaDB
- Instalação e configuração
- Comandos SQL básicos
- CRUD operations
- Joins e relacionamentos

MongoDB
- Instalação e configuração
- Estrutura de documentos
- CRUD operations
- Agregações

ORM/ODM
- Sequelize (SQL)
- Mongoose (MongoDB)
- Modelos e migrations

AULA 6: DEPLOY DE APLICAÇÕES
-----------------------------
Preparação para Deploy
- Variáveis de ambiente
- Build e otimização
- Testes finais

Plataformas de Deploy
- Vercel (Frontend)
- Heroku (Backend)
- Netlify (Static)
- Railway (Full stack)

CI/CD Básico
- GitHub Actions
- Deploy automático
- Monitoramento

Segurança
- HTTPS
- Variáveis de ambiente
- Autenticação
- Proteção contra ataques

CONCLUSÃO
==========
Parabéns por completar o curso!
Você agora tem conhecimento para criar aplicações web completas.

Próximos passos:
- Praticar com projetos reais
- Contribuir com open source
- Continuar aprendendo
- Especializar-se em áreas específicas`,
        conteudo: [
            "HTML5 e CSS3 Fundamentos",
            "JavaScript ES6+",
            "React.js do zero",
            "Node.js e Express",
            "Banco de Dados",
            "Deploy de aplicações"
        ]
    },
    {
        id: 2,
        titulo: "Marketing Digital",
        descricao: "Domine as estratégias de marketing digital para crescer seu negócio online.",
        preco: 70,
        categoria: "Marketing",
        duracao: "30 horas",
        nivel: "Iniciante",
        emoji: "📈",
        pixKey: "lucasklein2411@gmail.com",
        codigoAcesso: "MKT2024",
        alunos: 2,
        avaliacao: 10,
        conteudoTexto: `CURSO: MARKETING DIGITAL
========================

AULA 1: FUNDAMENTOS DE MARKETING
---------------------------------
O que é Marketing Digital
- Diferença entre marketing tradicional e digital
- Importância do marketing digital hoje
- Funil de vendas digital

Persona e Público-alvo
- Definindo sua persona
- Pesquisa de mercado
- Segmentação de público

Jornada do Cliente
- Etapas da jornada
- Touchpoints
- Mapeamento da jornada

AULA 2: REDES SOCIAIS
----------------------
Estratégia de Redes Sociais
- Escolhendo as redes certas
- Calendário de conteúdo
- Tom de voz e identidade

Instagram
- Perfil otimizado
- Stories e Reels
- Hashtags e alcance
- Engajamento

Facebook
- Página empresarial
- Grupos
- Facebook Ads básico
- Analytics

LinkedIn
- Perfil profissional
- Conteúdo B2B
- Networking
- LinkedIn Ads

TikTok
- Conteúdo viral
- Trends e desafios
- Algoritmo
- Monetização

AULA 3: SEO E TRÁFEGO PAGO
---------------------------
SEO (Search Engine Optimization)
- O que é SEO
- Palavras-chave
- On-page SEO
- Off-page SEO
- SEO técnico
- Ferramentas de SEO

Google Ads
- Campanhas de pesquisa
- Campanhas de display
- Remarketing
- Otimização de campanhas
- ROI e métricas

Facebook/Instagram Ads
- Criativos e copy
- Segmentação
- Pixel do Facebook
- Funis de anúncios
- Testes A/B

AULA 4: EMAIL MARKETING
-------------------------
Estratégia de Email Marketing
- Construção de lista
- Segmentação
- Automação
- Deliverability

Tipos de Email
- Newsletter
- Sequência de boas-vindas
- Emails promocionais
- Emails transacionais

Copywriting para Email
- Assuntos que convertem
- Estrutura do email
- CTA efetivos
- Testes A/B

Ferramentas
- Mailchimp
- ActiveCampaign
- ConvertKit
- Brevo (antigo Sendinblue)

AULA 5: COPYWRITING
-------------------
Fundamentos de Copywriting
- O que é copywriting
- Psicologia da persuasão
- Gatilhos mentais
- Storytelling

Estruturas de Copy
- AIDA (Atenção, Interesse, Desejo, Ação)
- PAS (Problema, Agitação, Solução)
- FAB (Features, Advantages, Benefits)
- 4 U's (Urgent, Unique, Useful, Ultra-specific)

Copy para diferentes canais
- Landing pages
- Anúncios
- Redes sociais
- Emails
- Blog posts

Testes e Otimização
- Testes A/B
- Métricas de copy
- Otimização contínua

AULA 6: ANALYTICS
------------------
Google Analytics
- Instalação e configuração
- Métricas principais
- GA4 vs Universal Analytics
- Relatórios personalizados

Métricas Importantes
- Tráfego
- Taxa de conversão
- CAC (Custo de Aquisição)
- LTV (Lifetime Value)
- ROI (Return on Investment)

Ferramentas de Analytics
- Google Search Console
- Hotjar
- Crazy Egg
- SEMrush

Análise de Dados
- Interpretando métricas
- Tomada de decisão baseada em dados
- Relatórios e dashboards

CONCLUSÃO
==========
Parabéns por completar o curso!
Você agora tem conhecimento para criar estratégias de marketing digital completas.

Próximos passos:
- Implementar estratégias
- Testar e otimizar
- Acompanhar resultados
- Continuar aprendendo`,
        conteudo: [
            "Fundamentos de Marketing",
            "Redes Sociais",
            "SEO e Tráfego Pago",
            "Email Marketing",
            "Copywriting",
            "Analytics"
        ]
    },
    {
        id: 3,
        titulo: "Design Gráfico",
        descricao: "Aprenda a criar designs profissionais com Photoshop, Illustrator e Figma.",
        preco: 50,
        categoria: "Design",
        duracao: "35 horas",
        nivel: "Intermediário",
        emoji: "🎨",
        pixKey: "lucasklein2411@gmail.com",
        codigoAcesso: "DES2024",
        alunos: 2,
        avaliacao: 10,
        conteudoTexto: `CURSO: DESIGN GRÁFICO
=====================

AULA 1: TEORIA DAS CORES
-------------------------
Fundamentos da Cor
- O que é cor
- Círculo cromático
- Cores primárias, secundárias e terciárias
- Temperatura das cores (quentes e frias)

Harmonia de Cores
- Monocromático
- Análogo
- Complementar
- Triádico
- Tetrádico

Psicologia das Cores
- Significado das cores
- Cores e emoções
- Cores em branding
- Testes A/B com cores

Paletas de Cores
- Como criar paletas
- Ferramentas de cor
- Paletas populares
- Acessibilidade e contraste

AULA 2: PHOTOSHOP AVANÇADO
---------------------------
Interface do Photoshop
- Workspace personalizado
- Painéis e ferramentas
- Atalhos essenciais

Camadas e Máscaras
- Trabalhando com layers
- Layer masks
- Blending modes
- Smart objects

Seleção e Recorte
- Ferramentas de seleção
- Select e mask
- Pen tool avançado
- Recorte preciso

Ajustes e Filtros
- Adjustment layers
- Filtros não-destrutivos
- Camera Raw
- Color grading

Retoque e Edição
- Clone stamp
- Healing brush
- Content-aware fill
- Frequency separation

AULA 3: ILLUSTRATOR
-------------------
Interface do Illustrator
- Workspace e painéis
- Ferramentas vetoriais
- Atalhos essenciais

Formas e Vetores
- Shape tools
- Pen tool avançado
- Pathfinder
- Shape builder

Tipografia
- Fontes e classificação
- Kerning, tracking e leading
- Hierarquia tipográfica
- Texto em caminho

Cores e Gradientes
- Swatches
- Gradientes
- Padrões
- Cores globais

Ilustração
- Desenho vetorial
- Brush tool
- Blob brush
- Ilustração de ícones

AULA 4: FIGMA PARA UI/UX
--------------------------
Introdução ao Figma
- Interface do Figma
- Frames e artboards
- Componentes
- Auto layout

Design de Interface
- Grid systems
- Espaçamento e consistência
- Tipografia digital
- Ícones e ilustrações

Prototipagem
- Interactive components
- Prototypes e links
- Animações
- Smart animate

Design Systems
- Componentes reutilizáveis
- Estilos globais
- Variáveis
- Documentação

Colaboração
- Compartilhamento
- Comentários
- Versionamento
- Dev mode

AULA 5: BRANDING
-----------------
Identidade Visual
- O que é branding
- Importância da identidade visual
- Pesquisa e briefing

Logotipos
- Tipos de logos
- Processo de criação
- Versões do logo
- Grid de construção

Paleta de Branding
- Cores da marca
- Cores secundárias
- Cores neutras
- Aplicação das cores

Tipografia de Marca
- Fontes primárias e secundárias
- Escala tipográfica
- Aplicação em diferentes contextos

Manual de Marca
- Guia de uso
- Do's and don'ts
- Aplicações
- Versões do logo

AULA 6: DESIGN PARA REDES SOCIAIS
----------------------------------
Formatos para Redes Sociais
- Dimensões por plataforma
- Formatos de imagem
- Vídeo vs imagem estática
- Stories e posts

Instagram
- Grid perfeito
- Stories e carrossel
- Reels e vídeos
- Highlights

Facebook
- Posts e capa
- Anúncios
- Grupos
- Página empresarial

LinkedIn
- Posts e artigos
- Banners
- Infográficos
- Apresentações

TikTok
- Thumbnails
- Overlays
- Texto em vídeo
- Efeitos visuais

Ferramentas
- Canva
- Adobe Express
- Unsplash
- Pexels

CONCLUSÃO
==========
Parabéns por completar o curso!
Você agora tem conhecimento para criar designs profissionais para diversos contextos.

Próximos passos:
- Construir portfólio
- Praticar com projetos reais
- Especializar-se em uma área
- Continuar aprendendo`,
        conteudo: [
            "Teoria das Cores",
            "Photoshop Avançado",
            "Illustrator",
            "Figma para UI/UX",
            "Branding",
            "Design para Redes Sociais"
        ]
    },
    {
        id: 4,
        titulo: "Finanças Pessoais",
        descricao: "Aprenda a gerenciar seu dinheiro, investir e construir patrimônio.",
        preco: 40,
        categoria: "Finanças",
        duracao: "20 horas",
        nivel: "Iniciante",
        emoji: "💰",
        pixKey: "lucasklein2411@gmail.com",
        codigoAcesso: "FIN2024",
        alunos: 2,
        avaliacao: 10,
        conteudoTexto: `CURSO: FINANÇAS PESSOAIS
========================

AULA 1: ORÇAMENTO PESSOAL
--------------------------
O que é Orçamento
- Importância do orçamento
- Diferença entre ganhos e gastos
- Controle financeiro

Métodos de Orçamento
- Método 50/30/20
- Método envelope
- Orçamento zero
- Apps de controle

Rastreamento de Gastos
- Como registrar gastos
- Categorização
- Identificação de vazamentos
- Análise de padrões

Ferramentas
- Planilhas
- Apps financeiros
- Extratos bancários
- Automatização

AULA 2: ELIMINAÇÃO DE DÍVIDAS
------------------------------
Tipos de Dívidas
- Dívidas boas vs ruins
- Juros compostos
- Cartão de crédito
- Empréstimos

Métodos de Pagamento
- Método bola de neve
- Método avalanche
- Negociação com credores
- Refinanciamento

Prevenção de Dívidas
- Controle de impulsos
- Reserva de emergência
- Planejamento de compras
- Educação financeira

Recuperação de Crédito
- O que é score de crédito
- Como melhorar o score
- Limpeza de nome
- Reconstrução de crédito

AULA 3: FUNDAMENTOS DE INVESTIMENTOS
-------------------------------------
Introdução a Investimentos
- O que é investir
- Diferença entre poupar e investir
- Risco vs retorno
- Horizonte de investimento

Perfil do Investidor
- Conservador
- Moderado
- Arrojado
- Teste de perfil

Conceitos Básicos
- Inflação
- Juros simples e compostos
- Liquidez
- Diversificação

Produtos de Investimento
- Renda fixa
- Renda variável
- Fundos de investimento
- Tesouro Direto

AULA 4: RENDA FIXA E VARIÁVEL
------------------------------
Renda Fixa
- O que é renda fixa
- Tesouro Direto
- CDB e LCI/LCA
- Debêntures

Renda Variável
- Ações
- ETFs
- Fundos imobiliários
- Criptomoedas

Comparação
- Riscos e retornos
- Tributação
- Liquidez
- Adequação ao perfil

Estratégias
- Alocação de ativos
- Rebalanceamento
- Diversificação
- Longo prazo

AULA 5: PLANEJAMENTO FINANCEIRO
---------------------------------
Objetivos Financeiros
- Definição de metas
- Curto, médio e longo prazo
- SMART goals
- Priorização

Reserva de Emergência
- O que é e importância
- Quanto guardar
- Onde guardar
- Quando usar

Aposentadoria
- Previdência social vs privada
- PGBL vs VGBL
- Cálculo de aposentadoria
- Estratégias de acumulação

Proteção Patrimonial
- Seguros
- Testamento
- Inventário
- Planejamento sucessório

AULA 6: APOSENTADORIA
----------------------
Previdência Social
- Regras do INSS
- Idade mínima
- Tempo de contribuição
- Cálculo do benefício

Previdência Privada
- PGBL (dedutível no IR)
- VGBL (não dedutível)
- Taxas de administração
- Rentabilidade

Outras Fontes de Renda
- Renda passiva
- Investimentos
- Imóveis
- Negócios

Planejamento
- Cálculo de necessidade
- Estratégias de acumulação
- Fase de acumulação vs decumulação
- Ajustes ao longo do tempo

CONCLUSÃO
==========
Parabéns por completar o curso!
Você agora tem conhecimento para gerenciar suas finanças e construir patrimônio.

Próximos passos:
- Implementar o que aprendeu
- Continuar educando-se
- Buscar aconselhamento profissional se necessário
- Revisar planos regularmente`,
        conteudo: [
            "Orçamento Pessoal",
            "Eliminação de Dívidas",
            "Fundamentos de Investimentos",
            "Renda Fixa e Variável",
            "Planejamento Financeiro",
            "Aposentadoria"
        ]
    },
    {
        id: 5,
        titulo: "Inglês para Negócios",
        descricao: "Fluência em inglês focado em ambiente corporativo e negociações.",
        preco: 70,
        categoria: "Idiomas",
        duracao: "50 horas",
        nivel: "Intermediário",
        emoji: "🌍",
        pixKey: "lucasklein2411@gmail.com",
        codigoAcesso: "ING2024",
        alunos: 2,
        avaliacao: 10,
        conteudoTexto: `CURSO: INGLÊS PARA NEGÓCIOS
===========================

AULA 1: VOCABULÁRIO CORPORATIVO
--------------------------------
Termos Essenciais
- Palavras comuns em negócios
- Abreviações (CEO, ROI, KPI, etc)
- Expressões idiomáticas
- Falsos cognatos

Reuniões
- Vocabulário de reuniões
- Frases para participar
- Opiniões e sugestões
- Concordância e discordância

Apresentações
- Estrutura de apresentação
- Transições
- Introduções e conclusões
- Perguntas e respostas

Emails Profissionais
- Abertura e fechamento
- Frases formais
- Solicitações
- Confirmações

AULA 2: APRESENTAÇÕES EM INGLÊS
---------------------------------
Estrutura de Apresentação
- Hook (gancho)
- Agenda
- Conteúdo principal
- Call to action

Introdução
- Apresentação pessoal
- Objetivo da apresentação
- Agenda
- Regras de engajamento

Desenvolvimento
- Organização de ideias
- Uso de exemplos
- Dados e estatísticas
- Storytelling

Conclusão
- Resumo dos pontos
- Call to action
- Perguntas e respostas
- Agradecimentos

Técnicas de Apresentação
- Linguagem corporal
- Contato visual
- Gerenciamento de tempo
- Lidar com nervosismo

AULA 3: NEGOCIAÇÕES
--------------------
Preparação para Negociação
- Pesquisa e planejamento
- Definição de objetivos
- Conhecimento da contraparte
- Estratégias

Durante a Negociação
- Abertura
- Propostas e contrapropostas
- Concessões
- Fechamento

Frases de Negociação
- Making offers
- Responding to offers
- Compromises
- Deadlines

Cultura de Negociação
- Diferenças culturais
- Etiqueta internacional
- Adaptação ao contexto
- Sensibilidade cultural

AULA 4: EMAILS PROFISSIONAIS
-----------------------------
Estrutura de Email Profissional
- Subject line efetivo
- Abertura apropriada
- Corpo do email
- Fechamento profissional

Tipos de Emails
- Solicitações
- Confirmações
- Reclamações
- Agradecimentos
- Follow-ups

Tom e Estilo
- Formal vs informal
- Clareza e concisão
- Polidez profissional
- Evitando mal-entendidos

Gramática e Vocabulário
- Tempos verbais apropriados
- Expressões formais
- Evitando gírias
- Precisão linguística

AULA 5: REUNIÕES E CONFERÊNCIAS
--------------------------------
Tipos de Reuniões
- Reuniões de equipe
- Reuniões com clientes
- Conferências
- Videoconferências

Participação em Reuniões
- Intervenção apropriada
- Fazendo perguntas
- Clarificando pontos
- Contribuindo ideias

Liderança de Reuniões
- Facilitação
- Gerenciamento de tempo
- Manter foco
- Conclusão e follow-up

Videoconferências
- Etiqueta virtual
- Ferramentas digitais
- Gerenciamento técnico
- Engajamento remoto

AULA 6: CULTURA EMPRESARIAL
----------------------------
Cultura Corporativa
- Valores organizacionais
- Normas e expectativas
- Comunicação interna
- Dinâmicas de poder

Networking Profissional
- Small talk profissional
- Construção de relacionamentos
- Follow-up
- Manutenção de contatos

Etiqueta Internacional
- Cumprimentos
- Títulos e formas de tratamento
- Presentes corporativos
- Refeições de negócios

Comunicação Intercultural
- Diferenças de comunicação
- Alta vs baixa contexto
- Estilos de comunicação
- Adaptação cultural

CONCLUSÃO
==========
Parabéns por completar o curso!
Você agora tem conhecimento para se comunicar profissionalmente em inglês em contextos de negócios.

Próximos passos:
- Praticar regularmente
- Imersão no idioma
- Participar de eventos internacionais
- Continuar estudando`,
        conteudo: [
            "Vocabulário Corporativo",
            "Apresentações em Inglês",
            "Negociações",
            "Emails Profissionais",
            "Reuniões e Conferências",
            "Cultura Empresarial"
        ]
    },
    {
        id: 6,
        titulo: "Produção de Vídeo",
        descricao: "Crie vídeos profissionais para YouTube, Instagram e redes sociais.",
        preco: 50,
        categoria: "Vídeo",
        duracao: "25 horas",
        nivel: "Iniciante",
        emoji: "🎬",
        pixKey: "lucasklein2411@gmail.com",
        codigoAcesso: "VID2024",
        alunos: 2,
        avaliacao: 10,
        conteudoTexto: `CURSO: PRODUÇÃO DE VÍDEO
=========================

AULA 1: ROTEIRO E STORYBOARD
------------------------------
Estrutura de Roteiro
- Introdução (hook)
- Desenvolvimento
- Clímax
- Conclusão
- Call to action

Tipos de Roteiro
- Roteiro para YouTube
- Roteiro para Instagram
- Roteiro para TikTok
- Roteiro corporativo

Storyboard
- O que é storyboard
- Como criar storyboard
- Planejamento de cenas
- Visualização do projeto

Técnicas de Storytelling
- Narrativa envolvente
- Personagem e conflito
- Arco narrativo
- Emoção e conexão

AULA 2: ILUMINAÇÃO E ÁUDIO
---------------------------
Iluminação Básica
- Luz natural vs artificial
- Três pontos de luz
- Temperatura de cor
- Reflectores e difusores

Equipamentos de Iluminação
- Softboxes
- Ring lights
- Panelas LED
- Luzes práticas

Áudio
- Microfones (lapela, shotgun, handheld)
- Gravadores de áudio
- Ambiente acústico
- Redução de ruído

Técnicas de Gravação
- Posicionamento de microfone
- Níveis de áudio
- Monitoramento
- Backup de áudio

AULA 3: EDIÇÃO COM PREMIERE
-----------------------------
Interface do Premiere
- Workspace
- Painéis
- Timeline
- Ferramentas essenciais

Importação e Organização
- Importar mídia
- Organizar bins
- Criar proxies
- Metadados

Corte e Edição
- Ferramentas de corte
- Ripple edit
- Slip e slide
- Trim tools

Transições e Efeitos
- Transições básicas
- Efeitos de vídeo
- Efeitos de áudio
- Keyframing

Texto e Gráficos
- Títulos
- Lower thirds
- Gráficos animados
- Logotipos

AULA 4: EFEITOS VISUAIS
------------------------
Color Grading
- Correção de cor
- Color grading
- LUTs
- Estilos de cor

Efeitos Especiais
- Chroma key (green screen)
- Mascaramento
- Rastreamento de movimento
- Composição

Animação
- Motion graphics básico
- Animação de texto
- Transições animadas
- Efeitos de partículas

Técnicas Avançadas
- Composição multicam
- Sincronização
- Estabilização
- Remoção de objetos

AULA 5: YOUTUBE SEO
--------------------
Otimização de Títulos
- Palavras-chave
- Títulos clicáveis
- Comprimento ideal
- Testes A/B

Descrições
- Estrutura ideal
- Palavras-chave
- Links e CTAs
- Hashtags

Thumbnails
- Importância da thumbnail
- Design efetivo
- Cores e contraste
- Testes A/B

Tags e Metadados
- Pesquisa de palavras-chave
- Tags relevantes
- Categorias
- Playlists

Analytics
- Métricas importantes
- Retenção de audiência
- CTR (Click-through rate)
- Tempo de assistir

AULA 6: MONETIZAÇÃO
--------------------
Programa de Parcerias YouTube
- Requisitos
- Tipos de monetização
- RPM vs CPM
- Fatores que afetam ganhos

Outras Plataformas
- Instagram (IGTV, Reels)
- TikTok (Creator Fund)
- Twitch
- Patreon

Patrocínios
- Como conseguir patrocinadores
- Tipos de parcerias
- Disclosure transparente
- Negociação

Venda de Produtos
- Merchandise
- Cursos e produtos digitais
- Afiliados
- Consultoria

Estratégias de Crescimento
- Consistência
- Engajamento com comunidade
- Colaborações
- Cross-promoção

CONCLUSÃO
==========
Parabéns por completar o curso!
Você agora tem conhecimento para produzir vídeos profissionais para diversas plataformas.

Próximos passos:
- Praticar regularmente
- Construir portfólio
- Analisar resultados
- Continuar aprendendo novas técnicas`,
        conteudo: [
            "Roteiro e Storyboard",
            "Iluminação e Áudio",
            "Edição com Premiere",
            "Efeitos Visuais",
            "YouTube SEO",
            "Monetização"
        ]
    }
];
